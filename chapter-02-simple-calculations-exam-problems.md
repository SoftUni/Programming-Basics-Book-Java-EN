...
After we have built an idea on how to solve the task and we have chosen the data types, it is time to get to **write the code**. As in the previous tasks, we can divide the solution into three smaller tasks: 
...
The task does not specify special formatting or rounding of the result, so we just have to calculate the output and print it on the console. 

![](assets/chapter-2-2-images/04.Money-04.png)

Let's pay attention to something that applies to all other tasks of this type: written like that, the solution of the task is pretty detailed. As the task is not too complex, we could write one complex expression, where after we read the input data, we calculate the output. For example, such an expression would look like this:
